...
import 'package:the_doctarine_of_the_ppl_of_the_quran/system/widgets/grids/exams_teachers/exam.dart';
...
class ExamTeachersScreen extends StatefulWidget {
  const ExamTeachersScreen({super.key});

  @override
  State<ExamTeachersScreen> createState() => _ExamTeachersScreenState();
}

class _ExamTeachersScreenState extends State<ExamTeachersScreen> {
  Duration duration = const Duration(seconds: 5);
  bool minimumLoadTimeCompleted = false;
  late ExamTeacherController controller;

  final Rxn<ExamTeacherInfoDialog> examRecord = Rxn<ExamTeacherInfoDialog>();
  final Duration delay = const Duration(seconds: 5);
  late EditExamTeachers editController;
  final RxBool hasSelection = false.obs;

  @override
  void initState() {
    super.initState();
    controller = Get.find<ExamTeacherController>();
    _loadData();
  }
  ...
  void dispose() {
    controller.dispose();
    Get.delete<ExamTeacherController>();
    super.dispose();
  }
    ...
    return Column(
      children: [
        Padding(
          ...
          child: TopButtons(
            onAdd: () {
              if (Get.isRegistered<EditExamTeachers>()) {
                hasSelection.value = false;
                examRecord.value = null;
                Get.delete<EditExamTeachers>();
              }
              Get.put(form.FormController(14));
              Get.dialog(ExamTeachersDialog());
            },
            onEdit: () {
              if (examRecord.value != null) {
                Get.put(form.FormController(14));
                editController.updateExamTeacher(examRecord.value!);
            ...
            onDelete: () async {
              ...
              await ApiService.delete(ApiEndpoints.getAccountInfoById(
                  examRecord.value?.techer.teacherAccountId ?? 0));
              setState(() {
                examRecord.value = null;
                hasSelection.value = false;
                _loadData();
        ...
        Expanded(
          child: Obx(() {
            ...
            if (controller.teachersList.isEmpty) {
              return ErrorIllustration(
